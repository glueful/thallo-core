...
/**
 * Grant the commerce permission to Aegis's `administrator` role. The
 * `glueful/thallo-commerce` pack declares the permission row (its own seed migration);
 * granting onto roles is the HOST APP's decision — this dependent migration runs after
 * Aegis has created the role.
 */
final class GrantCommercePermissionsToAdministrator implements MigrationInterface
{
    ...
    {
        return 'Grant commerce.manage to the administrator role.';
    }

    /**
     * Insert any missing commerce permission rows; return slug => uuid.
     *
        ...
        foreach (self::PERMISSIONS as $slug => $label) {
            if (!isset($bySlug[$slug])) {
                ...
                $insert[] = [
                    ...
                    'category' => 'commerce', 'description' => $label, 'is_system' => true,
                ];
            }
        }
        if ($insert !== []) {
