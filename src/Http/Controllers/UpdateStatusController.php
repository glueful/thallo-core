 ...
 * Read-only, for any signed-in admin user (the version shows in the user menu) — never on the
 * unauthenticated /admin/config, where the installed version would be a fingerprint. The upgrade
 * itself is a server-side command.
 */
final class UpdateStatusController
{
    ...
    #[ApiOperation(
        ...
        description: 'The installed glueful/thallo-core version, the newest published version this install '
            . 'may move to (from the daily Packagist check), and the release notes link. Read-only, for any '
            . 'signed-in admin user. The upgrade is `composer update && php glueful thallo:provision`.',
        tags: ['Utilities'],
    )]
